...
class SapoProvider extends BaseVideoProvider
{
     ...
     * {@inheritdoc}
     */
    protected function getMetadataThumbnail(MediaInterface $media, $url)
    {
        ...
        $html = new \Symfony\Component\DomCrawler\Crawler($response->getContent());
        $metadata = [];
        $thumbnail_node = $html->filter('link[itemprop="thumbnailUrl"]');
    ...
    {
        $thumbnail_url = sprintf('http://videos.sapo.pt/%s', $media->getProviderReference());
        $url = sprintf('http://videos.sapo.pt/oembed?url=http://videos.sapo.pt/%s&format=json', $media->getProviderReference());

        try {
            $metadata = $this->getMetadata($media, $url);
            $metadata_thumbnail = $this->getMetadataThumbnail($media, $thumbnail_url);
